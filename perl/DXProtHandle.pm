...
our $remove_auto_ftx = 1;		# remove all auto generated FTx spots
...
#	my ($hops) = $pc->[8] =~ /^H(\d+)/;
	...
	# if required (default: yes) remove all FTx spots that are (or appear to be) autogenerated
	if ($remove_auto_ftx && $pc->[5] =~ /^\s*FT[48]\s+(?:db|hz|sent|rcvd)?[\-\+]?\d+(?:db|hz)?/i) {
		dbg($line) if isdbg('nologchan');
