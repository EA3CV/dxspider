...
if($p->{action}=~/^reg_(?:pending|history|search|accept|reject)$/){ws_send_guarded($cid,encode_json({type=>$p->{action}.'_result',status=>$msg->{status}//'error',messages=>$msg->{messages}||[],error=>$msg->{error},result=>$msg->{result}}));return}
